# JS PROGRESS

# Phase I (15/08/25 - 23/08/25) 8 Days
1. Introduction
2. Variables (var, let, const)
3. Declaration & Initialization
4. Scope (Global , Functional , Block)
5. Reassigned but not Redeclared
6. Temporal Dead Zone
7. Data Types & System Types
8. Operators (typeof & instanceof)
9. Control Flow  (if,else,else-if,switchcase,
    early return pattern)
10. Loops (for,while,do-while) 
11. Break & Continue
12. Functions ( 3 types including arrow ) 
13. Parameters & Arguments 
14. Default Parameters
15. Rest for taking multiple arguments
16. Return value and early return
17. First Class functions 
18. High order function
19. Pure and Impure Functions
20. Clousers
21. Lexical scope
22. IIFE
23. Hoisting difference between declaration and expression
24. Arrays 
25. Method of Arrays
26. Destructuring , spread operator 
27. Objects

# Phase II (23/08/25 - 30/08/25) ? Days

1. DOM
2. Events and Event Handling
3. Forms and Form Validation
4. Timers And Intervals
5. Local Storage And Session Storage And Cookies
6. PROJECTS (FINAL)